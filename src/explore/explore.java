...
//		}
	...
	public void keyPressed(KeyEvent e) {
		//if(!end) {
        	...
        	if (e.getKeyCode()==KeyEvent.VK_DOWN && p1.y<size-1 && maze.get(p1.y).get(p1.x).get(1)!=1) {// && maze.get(p1.y+1).get(p1.x).get(0)!=1) {
        		p1.y+=1;
        	}
        	if (e.getKeyCode()==KeyEvent.VK_RIGHT && p1.x<size-1 && maze.get(p1.y).get(p1.x).get(2)!=1) {// && maze.get(p1.y).get(p1.x+1).get(3)!=1) {
        		p1.x+=1;
        	}
        	if (e.getKeyCode()==KeyEvent.VK_LEFT && p1.x>0 && maze.get(p1.y).get(p1.x).get(3)!=1) {// && maze.get(p1.y).get(p1.x-1).get(2)!=1) {
        		p1.x-=1;
        	}
			if (e.getKeyCode()==KeyEvent.VK_W && p2.y>0 && maze.get(p2.y).get(p2.x).get(0)!=1) {// && maze.get(p1.y-1).get(p1.x).get(1)!=1) {
        		p2.y-=1;
        	}
        	if (e.getKeyCode()==KeyEvent.VK_S && p2.y<size-1 && maze.get(p2.y).get(p2.x).get(1)!=1) {// && maze.get(p1.y+1).get(p1.x).get(0)!=1) {
        		p2.y+=1;
        	}
        	if (e.getKeyCode()==KeyEvent.VK_D && p2.x<size-1 && maze.get(p2.y).get(p2.x).get(2)!=1) {// && maze.get(p1.y).get(p1.x+1).get(3)!=1) {
        		p2.x+=1;
        	}
