...
public class autoSolve extends JComponent implements Runnable, KeyListener, MouseListener{
/**
	 * 
	 */
...
//		g2D.fill(new Rectangle2D.Double((size-1)*thick+thick/8,(size-1)*thick+thick/8,3*thick/4,3*thick/4));
		...
		if(solution) {
			run();
			p1.x = pathJ.get(moveJ);
			p1.y = pathI.get(moveI);
			if(moveI<pathI.size()-1&&moveJ<pathJ.size()-1) {
				moveI++;
				moveJ++;
			}
		}
